...
async function runSQL(query) {
    console.log(`[DEBUG] Executing SQL: ${query}`); // Added for debugging
...
`,
    tools: { functionDeclarations: [{ name: "get_table_data", description: "Fetch rows from a table to get current game state information. Use this for reading data.", parameters: { type: "object", properties: { table: { type: "string" }, query: { type: "string", description: "A SQL WHERE clause to filter the results, e.g., \"name = 'Tara'\"" } }, required: ["table"] } }, { name: "create_npc", description: "Creates a new non-player character in the database.", parameters: { type: "object", properties: { name: { type: "string", description: "The character's name." }, description: { type: "string", description: "A brief description of the character." }, disposition: { type: "integer", description: "A number from -100 (hostile) to 100 (friendly)." }, location: { type: "string", description: "The current location of the character." }, is_hostile: { type: "boolean", description: "Whether the character is immediately hostile." } }, required: ["name", "description", "disposition", "location", "is_hostile"] } }, { name: "update_npc_data", description: "Updates data for an existing NPC.", parameters: { type: "object", properties: { npc_name: { type: "string", description: "The name of the NPC to update." }, new_location: { type: "string" }, new_description: { type: "string" }, new_disposition: { type: "integer" } }, required: ["npc_name"] } }] }
});
